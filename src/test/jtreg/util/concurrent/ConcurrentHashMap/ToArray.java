...
public class ToArray {
    ...
    static void executeTest() throws Throwable {
        ...
        final int maxWorkers = Math.max(2, Math.min(32, nCPU));
        final int nWorkers = ThreadLocalRandom.current().nextInt(2, maxWorkers + 1);
        ...
        CompletableFuture<?> foreman = CompletableFuture.runAsync(new Runnable() {
            ...
            private boolean checkProgress(Object[] a) {
                int size = a.length;
                if (size < prevSize) throw new AssertionError("WRONG WAY");
                if (size > maxSize) throw new AssertionError("OVERSHOOT");
                prevSize = size;
                return size == maxSize;
            }
            ...
            public void run() {
                ...
                boolean done;
                do {
                    done = checkProgress(m.values().toArray())
